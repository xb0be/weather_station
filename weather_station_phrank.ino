/*
Sample data, captured data on 5.2.2017:
$ rtl_433 -D -A -R 16
...
/*
  Have to wait for 433 MHz Arduino transmiter to verify this ...
  
  // Send start sequence (only one "0") and pause for 10us
  mySwitch.send("0");
  delayMicroseconds(10);
  // Send the signal with data 9 times, pause for 10us between them
  mySwitch.send(str_to_send);
  delayMicroseconds(10);
  mySwitch.send(str_to_send);
  delayMicroseconds(10);
  mySwitch.send(str_to_send);
  ...
  delayMicroseconds(10);
*/  
  delay(5000);                          // let's wait a little bit and repeat the phrank ;)
